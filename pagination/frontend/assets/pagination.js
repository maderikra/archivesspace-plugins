...
//when on the staff side, we must determine which controller we're on (on public side, all browsing is done on the search controller)
...
while(params[0].charAt(0) === '?'){params[0] = params[0].substr(1);}
params = "&" + params.join("&");
...
//check to see if the controller has paramaters in it already
if (controller.indexOf("?") > -1) {
	//checks to see if there is already a "page" field in the parameters; if so, strip it out of the existing URL and replace it with the revised page numbers
	...
	//if there is no page number field
	else 
    	{
...
$("#tabledSearchResults").before("<div class='pagination pagination-small pagination-centered'><ul class='newpages'></ul></div>");
